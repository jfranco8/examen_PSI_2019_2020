...
from datamodel.models import Game, Move, GameStatus, Counter
...
def test_query():
    ...
    move = Move.objects.create(game=game, player=user10, origin=game.cat2, target=11)
    game.full_clean()
    game.save()
    print(game)

    print(" ---- Movemos el Mouse a la posición 52 ----")
    move = Move.objects.create(game=game, player=user11, origin=game.mouse, target=52)
    game.full_clean()
    game.save()
    print(game)
    ...
    # User.objects.filter(games_as_cat__cat_user__username=self.users[0].username).count()
test_query()
